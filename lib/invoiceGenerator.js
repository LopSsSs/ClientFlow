...
export function generateInvoicePDF(invoiceData, businessData) {
  ...
  doc.setFontSize(20)
  doc.text(businessData.name || 'ClientFlow', margin, 20, { maxWidth: contentWidth - 55 })
  ...
  doc.text(`€${subtotal.toFixed(2)}`, pageWidth - margin - 5, yPosition + 5, {
    align: 'right',
  })
  ...
  if (invoiceData.tax > 0) {
    ...
    doc.setTextColor(90, 90, 90)
    doc.text(`IVA (${taxRate}%):`, pageWidth - margin - 65, yPosition + 5, { align: 'right' })
  ...
  doc.text(`€${total.toFixed(2)}`, pageWidth - margin - 5, yPosition + 6, {
    align: 'right',
  })

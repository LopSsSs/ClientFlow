...
export async function generateInvoicePDF(invoiceData, businessData) {
  ...
  // CTA principal de pago: PayPal > Bizum > transferencia. Stripe ya no es
  // un método de cobro para los clientes finales de cada negocio — solo se
  // usa para las suscripciones de ClientFlow (lib/plans.ts). Si el negocio
  // no ha configurado ninguno de los tres, no se muestra ningún enlace.
  ...
  let extraY = yPosition + (primaryMethod ? 15 : 10)

...
export async function generateInvoicePDF(invoiceData, businessData) {
  ...
  // Métodos de pago (opcional, viene de business_settings)
  let extraY = yPosition + (invoiceData.id ? 15 : 10)
  ...
  if (hasPaymentInfo) {
    ...
    doc.setFontSize(9)
    doc.setFont(undefined, 'bold')
    doc.text('Métodos de pago:', margin, extraY)
    doc.setFont(undefined, 'normal')
    extraY += 6
    if (businessData.payment_paypal_email) {
      doc.text(`PayPal: ${businessData.payment_paypal_email}`, margin, extraY)
      extraY += 5
    }
    if (businessData.payment_bizum_phone) {
      doc.text(`Bizum: ${businessData.payment_bizum_phone}`, margin, extraY)
      extraY += 5
    }
    if (businessData.payment_transfer_enabled && businessData.payment_transfer_iban) {

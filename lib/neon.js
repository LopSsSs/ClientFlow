...
export async function getUserById(userId) {
  const result = await sql`
    SELECT id, email, created_at, email_verified FROM users WHERE id = ${userId}
  `
  return result[0]
}
...
// Protección básica contra fuerza bruta: bloquea la cuenta 15 minutos tras
// 5 intentos fallidos seguidos. Se resetea en cualquier login correcto.
...
// La comparación se hace en el propio Postgres (NOW()), no con el reloj del
// servidor de la app: dos relojes de máquina distintos pueden no coincidir,
// y aquí eso significaría dejar pasar un login que debería seguir bloqueado.
...
export async function updateBusiness(businessId, userId, data) {
  const result = await sql`
    ...
    SET name = ${data.name}, phone = ${data.phone || null}, whatsapp_number = ${data.whatsapp_number || null},
        logo_url = ${data.logo_url || null}, currency = ${data.currency || 'EUR'},
        default_hourly_rate = ${data.default_hourly_rate ?? null}, updated_at = NOW()
...
export async function updateClientCoordinates(clientId, businessId, latitude, longitude) {
  const result = await sql`
    UPDATE clients SET latitude = ${latitude}, longitude = ${longitude}, updated_at = NOW()
    WHERE id = ${clientId} AND business_id = ${businessId}
    RETURNING *
  `
  return result[0]
